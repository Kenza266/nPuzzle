...
public class Puzzle {
    ...
    public ArrayList<Transformation> getPath(List<Integer> p) {
        State state = this.start;
        ArrayList<Transformation> path = new ArrayList<>();

        for (int i = 0; i<p.size(); i++) {
            //loc = chromosome.getGene().get(i).getIndex(0);
            //locParent = node.getParent().getState().getIndex(0);
            state = state.apply(Arrays.asList(p.get(i))).getState();
            path.add(new Transformation(String.valueOf(state.getData()), String.valueOf(p.get(i))));

        }
        Collections.reverse(path);
        //path.forEach(e-> System.out.println(e.getState()));
        return path;
    }
    ...
    public Result solveGenetic(geneticSolver solver, int maxIteration) throws InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        solver.initPopulation();
        long startTime = System.nanoTime();
        Chromosome solution = solver.solve(maxIteration);
        long endTime = System.nanoTime();
        long duration = (endTime - startTime); //divide by 1000000 to get milliseconds.
        if(solution == null){
            ...
            report = "Solved with AG\n" +
                    //"Report:\nNumber of opened nodes: " + count +
                    "\nExecution time: " + df.format((float) duration/1000000) + "ms";
                    //"\nSolution found at level: " + node.getLevel() +
                    //"\nMax nodes in memory: " + maxMem + "\n";
            return new Result(getPath(solution.getGene()), 0, duration, solution.getGene().size(), 0, report);
        }
    }

    public Result solvePSO() throws InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        PSOimplementation p = new PSOimplementation();
        long startTime = System.nanoTime();
        Particle solution = p.solve(this.start.getData());
        long endTime = System.nanoTime();
        long duration = (endTime - startTime); //divide by 1000000 to get milliseconds.
        if(solution == null){
            System.out.println("Not found\n");
            ...
            report = "Solved with PSO\n" +
                    //"Report:\nNumber of opened nodes: " + count +
                    "\nExecution time: " + df.format((float) duration/1000000) + "ms";
            //"\nSolution found at level: " + node.getLevel() +
            //"\nMax nodes in memory: " + maxMem + "\n";
            return new Result(getPath(solution.position), 0, duration, solution.position.size(), 0, report);
        }
    }
