...
public class MainWindowController {
    ...
    private static final List<String > algorithms = new ArrayList<>(Arrays.asList("BFS", "DFS", "Manhattan", "Euclid", "Hamming", "GA", "PSO"));
    ...
    private void initCheckBox() {
        checkBox.setValue("BFS");
        checkBox.getItems().addAll(algorithms);
    }
        ...
        run.setOnAction(e -> {
            ...
            if(validStateString(input.getText()) && validStateString(target.getText())){
                ...
                if(inversions%2 != 0)
                    ...
                    Result result;
                    try {
                        result = runAlgorithm(checkBox.getValue(), inversions);
    ...
    private Result runAlgorithm(String algorithm, int inversions) throws InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        ...
        switch (algorithm){
            ...
            case "GA":
                ...
                result = puzzle.solveGenetic(solver, 600);
                break;
            case "PSO":
    ...
    private TranslateTransition move(String i, String state) {
        TranslateTransition transition = new TranslateTransition();
        int zeroIndex = textList.indexOf("0");
        switch (i){
            case "-3":
