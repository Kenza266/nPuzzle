...
public class Puzzle {
    ...
    public ArrayList<Transformation> getPath(Chromosome chromosome) {
        //int loc, locParent;
        State state = this.start;
        List<Integer> p = chromosome.getGene();
        ArrayList<Transformation> path = new ArrayList<>();
    ...
    public Result solveGenetic(geneticSolver solver, int maxIteration) throws InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        ...
        if(solution == null){
            ...
            report = "Solved with AG\n" +
                    ...
                    //"\nSolution found at level: " + node.getLevel() +
                    //"\nMax nodes in memory: " + maxMem + "\n";
            return new Result(getPath(solution), 0, duration, 0, 0, report);
        }
    }
