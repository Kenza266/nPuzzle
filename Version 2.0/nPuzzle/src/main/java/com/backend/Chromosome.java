...
public class Chromosome {
    ...
    public int getInversions(State start, State goal) {
        ...
        for(int i=(int) Math.pow(10, 9-1); i>0; i=i/10) {
    ...
    public void createGene(State state, int m) {
        int index;
        List<Integer> possibilities;
        this.gene = new ArrayList<>(m);
        this.state = state;

        for(int i = 0; i<m; i++) {
            possibilities = this.state.getPossibilities();
            index = (int)(Math.random() * possibilities.size());
            this.gene.add(possibilities.get(index));
            this.state = this.state.getChild(this.state, this.state.getIndex(0), this.state.getIndex(0)+possibilities.get(index));
        }
    }
